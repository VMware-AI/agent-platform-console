...
<template>
  <div class="sp">
    ...
    <div v-else-if="skills.length === 0" class="sp-em">
      ...
      <h3>暂无 Skill</h3>
      <p>创建 Skill，为 OpenClaw、OpenCode、Hermes 等智能体扩展企业级能力。</p>
      <button class="btn btn-p" @click="openNew">新增第一个 Skill</button>
    </div>

    <div v-else-if="filtered.length === 0" class="sp-em">
      <div class="em-icon em-icn--info">i</div>
      <h3>未找到符合条件的 Skill</h3>
      <p>{{ search ? `没有匹配「${search}」的 Skill` : '当前筛选条件下没有符合条件的 Skill' }}</p>
          ...
          <tr v-for="s in filtered" :key="s.id" :class="{ 'tr-sel': sel.has(s.id) }">
            ...
            <td class="th-act">
              <div class="td-act">
                ...
                <button class="btn-sm" v-if="s.packageUrl && isOfflineReady(s.packageUrl)" @click="openInstall(s)">安装到 Agent</button>
                <button class="btn-sm btn-sm--danger" @click="dt = s; doOpen = true">删除</button>
              </div>
            </td>
          </tr>
        </tbody>
...
.sp { display: flex; flex-direction: column; height: 100%; min-height: 0; padding: 24px 32px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #1a1a2e; background: #f5f6fa; }
.sp-h { display: flex; align-items: flex-start; justify-content: space-between; margin-bottom: 20px; flex-shrink: 0; }
...
.sp-tb { display: flex; align-items: center; gap: 10px; margin-bottom: 16px; flex-shrink: 0; flex-wrap: wrap; }
...
.sp-tbl { overflow-x: auto; flex: 1; min-height: 0; background: #fff; border-radius: 10px; border: 1px solid #e5e7eb; }
...
.th-act { text-align: right; width: 110px; }
...
.dr-bd { position: fixed; inset: 0; background: rgba(0,0,0,.35); z-index: 1000; animation: fadeIn .15s ease; }
...
@media (max-width: 768px) { .sp { padding: 12px; } .dr { width: 100vw; } }
</style>
