    ...
    <div class="br-layout">
      <div class="br-config">
        ...
        <cds-card class="card"><div class="cp"><h2>{{ locale.t('branding.theme') }}</h2>
          ...
          <label class="fld"><cds-checkbox><input type="checkbox" v-model="overlayEnabled"/></cds-checkbox><span>{{ locale.t('branding.overlayEnable') }}</span></label>
          <label class="fld"><span>{{ locale.t('branding.cardPos') }}</span><cds-select control-width="shrink"><select v-model="loginCardPosition"><option value="center">居中</option><option value="left">左侧</option><option value="right">右侧</option></select></cds-select></label>
        </div></cds-card>
