/**
 * E2E tests for AI Agent Deployment — 5 scenarios (v5)
 *
 * Run with:
 *   node /data/claude/ai-workstation-platform/frontend/e2e/deploy-agent-test-v5.cjs
 *
 * Prerequisites:
 ...
 *
 * 4 Deploy Scenarios (run sequentially via API login):
 *   1. Full clone single: IP 172.16.85.135, name=test-v5-01
 *   2. Instant clone single: IP 172.16.85.136, name=test-v5-02
 *   3. Full clone batch x3: start IP 172.16.85.140, prefix=test-v5-dlvm
 *   4. Instant clone batch x2: start IP 172.16.85.150, prefix=test-v5-ic
 *   5. Agent list screenshot
 *
 * Screenshots saved to /tmp/e2e-screenshots/v5-*.png
 */
 ...
 * Prefers ic-p-* prefix parent VMs.
 */
async function dialogSelectInstantParent(page) {
  ...
  // First, try to select an ic-p-* option
  const opts = await sel.locator('option').all();
  let selected = false;
  ...
  for (const o of opts) {
    ...
    const t = (await o.textContent() || '').trim();
    if (v && v !== '' && t.startsWith('ic-p-')) {
      await sel.selectOption(v);
      console.log(`   [INFO] Selected parent VM (ic-p-*): ${t}`);
      selected = true;
      break;
    }
  }
  ...
  if (!selected) {
    for (const o of opts) {
      ...
      if (v && t && v !== '') {
        await sel.selectOption(v);
        console.log(`   [INFO] Selected parent VM (fallback): ${t}`);
        selected = true;
        break;
      }
    }
...
async function fillCommonFields(page, ip) {
  ...
  console.log(`   [INFO] Filled common fields for ${ip}`);
}
...
async function scenario1(page) {
  console.log('\n===== SCENARIO 1: Full Clone + Single Deploy (172.16.85.135, name=test-v5-01) =====');
  ...
  // Override hostname to test-v5-01 (fillCommonFields uses test-172-16-85-135)
  await dialogHostname(page, 'test-v5-01');

  // Screenshot before submit
  await screenshot(page, 'v5-01.png');

  // Submit
  await dialogSubmit(page);
  console.log('   Submitted');

  // Wait 15 seconds for deployment to process
  console.log('   Waiting 15s for deployment...');
  await page.waitForTimeout(15000);

  // Screenshot after submit
  await screenshot(page, 'v5-01-result.png');

  // Close dialog if still open
  await dialogClose(page);

  return { pass: true, detail: 'Full clone single deploy name=test-v5-01 IP=172.16.85.135' };
}
...
async function scenario2(page) {
  console.log('\n===== SCENARIO 2: Instant Clone + Single Deploy (172.16.85.136, name=test-v5-02) =====');
  ...
  // Select existing parent VM (ic-p-* prefix)
  const parentFound = await dialogSelectInstantParent(page);
  if (parentFound) {
    console.log('   [INFO] Parent VM selected');
  } else {
  ...
  // Override hostname to test-v5-02
  await dialogHostname(page, 'test-v5-02');

  // Screenshot before submit
  await screenshot(page, 'v5-02.png');
  ...
  // Screenshot after submit
  await screenshot(page, 'v5-02-result.png');

  // Close dialog if still open
  await dialogClose(page);

  return { pass: true, detail: parentFound ? 'Instant clone with ic-p-* parent VM name=test-v5-02 IP=172.16.85.136' : 'Instant clone without parent VM name=test-v5-02 IP=172.16.85.136' };
}

/**
 * Scenario 3: Full Clone + Batch x3 + Prefix test-v5-dlvm + Start IP 172.16.85.140
 */
async function scenario3(page) {
  console.log('\n===== SCENARIO 3: Full Clone + Batch x3 (prefix=test-v5-dlvm, startIP=172.16.85.140) =====');
  ...
  // Fill batch config: count=3, prefix=test-v5-dlvm, startIP=172.16.85.140
  await dialogBatchInputs(page, 3, 'test-v5-dlvm', '172.16.85.140');
  ...
  // Screenshot before submit
  await screenshot(page, 'v5-03.png');
  ...
  // Screenshot after submit
  await screenshot(page, 'v5-03-result.png');

  // Close dialog if still open
  await dialogClose(page);

  return { pass: true, detail: 'Full clone batch x3 prefix=test-v5-dlvm startIP=172.16.85.140' };
}

/**
 * Scenario 4: Instant Clone + Batch x2 + Prefix test-v5-ic + Start IP 172.16.85.150
 */
async function scenario4(page) {
  console.log('\n===== SCENARIO 4: Instant Clone + Batch x2 (prefix=test-v5-ic, startIP=172.16.85.150) =====');
  ...
  // Screenshot before submit
  await screenshot(page, 'v5-04.png');
  ...
  // Screenshot after submit
  await screenshot(page, 'v5-04-result.png');

  // Close dialog if still open
  await dialogClose(page);

  return { pass: true, detail: parentFound ? 'Instant clone batch x2 with ic-p-* parent VM prefix=test-v5-ic startIP=172.16.85.150' : 'Instant clone batch x2 without parent VM prefix=test-v5-ic startIP=172.16.85.150' };
}
...
async function main() {
  ...
  console.log('===== E2E: Deploy Agent Tests (v5) =====\n');
  console.log('--- LOGIN ---');
  ...
  try {
    ...
    await page.waitForTimeout(2000);
    await screenshot(page, 'v5-agent-list.png');
  } catch (err) {
  ...
  // Summary
  console.log('\n' + '='.repeat(70));
  console.log('  E2E DEPLOY AGENT TEST RESULTS — 5 Scenarios');
